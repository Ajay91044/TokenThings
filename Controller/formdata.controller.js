...
let addFormdatas = async (req, res) => {
    try {
        ...
        let { fname, Email, Mobile, Pass, age } = req.body

        let isEmailAvailable = await FormdataCollection.findOne({ Email })

        if (!isEmailAvailable) {
            let IsMobileAvailable = await FormdataCollection.findOne({ Mobile })

            if (!IsMobileAvailable) {
                let user = await FormdataCollection.create({ fname, Email, Mobile, Pass, age })
